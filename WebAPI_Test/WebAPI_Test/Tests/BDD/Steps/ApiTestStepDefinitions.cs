...
using WebAPI_Test.API;
using WebAPI_Test.Model.Request;
using WebAPI_Test.Model.Response;
using WebAPI_Test.Utils;

namespace WebAPI_Test.Tests.BDD.Steps
{
    [Binding]
    public class ApiTestStepDefinitions
    {
        ...
        public async Task WhenSendingThePOSTRequestWithBookingInfoToTheWebsite()
        {
            ...
            //payload = JsonConvert.SerializeObject(_bookingBuilder);
        ...
        {
            var api = new ApiClient();
            //payload = JsonConvert.SerializeObject(_partialBookingBuilder);
